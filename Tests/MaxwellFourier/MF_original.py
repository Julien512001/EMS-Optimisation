...
def compute_forces(bx_grid, by_grid, bz_grid, X, Y, speed, back_iron=False, N=10, M=10, mur_plate=1):
    ...
    for n in range(-N, N + 1):
        ...
        for m in range(-M, M + 1):
            ...
            Sz = integ_cube(X, Y, bz_grid, 2 * n * np.pi / lc, 2 * m * np.pi / wc)
            Sx = integ_cube(X, Y, bx_grid, 2 * n * np.pi / lc, 2 * m * np.pi / wc)
            Sy = integ_cube(X, Y, by_grid, 2 * n * np.pi / lc, 2 * m * np.pi / wc)
...
forces = compute_forces(bx, by, bz, X, Y, 50, back_iron=False, mur_plate=1)